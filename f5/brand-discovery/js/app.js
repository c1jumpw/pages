...
(function () {
  ...
  var PREVIEW = location.protocol === 'file:' ||
    /(^|\.)(github\.io|vercel\.app)$/.test(location.hostname) ||
      ...
      (state.files[group] || []).forEach(function (fl) {
        ...
        list.appendChild(h('li', { class: 'file is-' + fl.status },
          ...
          h('button', { type: 'button', class: 'file-x', 'aria-label': 'Remove ' + fl.name, onclick: function () {
            if (fl.xhr) { try { fl.xhr.abort(); } catch (e) {} }
  ...
  function uploadOne(group, file, entry, redraw) {
    function fail(message) { entry.status = 'error'; entry.error = message; redraw(); }
    function previewDone() { entry.status = 'done'; entry.preview = true; entry.path = state.folder + '/' + group + '/preview-' + entry.name; redraw(); save(); }

    fetch(CFG.endpoints.upload, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ folder: state.folder, group: group, name: file.name, size: file.size, type: file.type })
    }).then(function (r) {
      return r.json().catch(function () { return {}; }).then(function (j) { return { r: r, j: j }; });
    }).then(function (res) {
      if (!res.r.ok) {
        if (PREVIEW && noBackend(res.r.status)) return previewDone();
        return fail(res.j.error || 'Upload failed. Try again.');
      }
      var xhr = new XMLHttpRequest();
      entry.xhr = xhr;
      xhr.open('PUT', res.j.uploadUrl);
      xhr.upload.onprogress = function (e) {
        if (e.lengthComputable) { entry.progress = (e.loaded / e.total) * 100; redraw(); }
      };
      xhr.onload = function () {
        entry.xhr = null;
        if (xhr.status >= 200 && xhr.status < 300) { entry.status = 'done'; entry.path = res.j.path; entry.progress = 100; redraw(); refreshNext(); save(); }
        else fail('Upload failed (' + xhr.status + '). Try again.');
      };
      xhr.onerror = function () { entry.xhr = null; fail('Network error. Try again.'); };
